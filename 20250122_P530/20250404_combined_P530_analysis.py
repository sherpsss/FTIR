...
# fig_mc, axs_mc = plt.subplots(nrows=2,ncols=1,sharex=True,figsize=(8,10))
fig_mc, axs_mc = plt.subplots(figsize=(8, 10))

axs_mc.set_ylabel("Absorption [a.u.]",fontsize=axislabelsfont)
...
axs_mc.tick_params(axis='x',labelsize=ticksize)
axs_mc.tick_params(axis='y',labelsize=ticksize)
...
for i in range(0,len(angles)):
    ...
    axs_mc.plot(angle_meas.TE_wavenum_masked, alpha_ISB, label= str(angle) + '$\degree$',
...
alpha_ISB = -np.log(angle_meas.TM_masked / angle_meas.TE_masked) + offset
axs_mc.plot(angle_meas.TE_wavenum_masked, alpha_ISB, label= str(angle) + '$\degree$',color=reds[3],linewidth=linesize)
...
fig_mc.suptitle('Superlattice MQW Sample Absorption',fontsize=titlesize)
axs_mc.legend(loc='upper right')
# axs_mc[1].legend(loc='upper right')
# axs_mc[2].legend(loc='upper right')
axs_mc.legend(prop={"size":legendfont})
...
# axs_mc.xaxis.set_major_locator(MaxNLocator(integer=True))
